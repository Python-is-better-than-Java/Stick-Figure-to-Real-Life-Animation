...
class DualNet(object):
    def __init__(self, sess, image_size=256, batch_size=1,gcn = 64, dcn=64, \
                 A_channels = 3, B_channels = 3, dataset_name='Dataset', \
    ...
    ###    define place holders
        self.real_A = tf.compat.v1.placeholder(tf.float32,[self.batch_size, self.image_size, self.image_size,
                                         self.A_channels ],name='real_A')
        self.real_B = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.image_size, self.image_size,
    ...
    def train(self, args):
        """Train Dual GAN"""
        decay = 0.9
        self.d_optim = tf.compat.v1.train.RMSPropOptimizer(args.lr, decay=decay) \
                          .minimize(self.d_loss, var_list=self.d_vars)
                          
        self.g_optim = tf.compat.v1.train.RMSPropOptimizer(args.lr, decay=decay) \
        ...
        self.writer = tf.compat.v1.summary.FileWriter("./logs/"+self.dir_name, self.sess.graph)
