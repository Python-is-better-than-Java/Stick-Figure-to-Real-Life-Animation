...
def batch_norm(x,  name="batch_norm"):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        scale = tf.compat.v1.get_variable("scale", [nchannels], initializer=tf.random_normal_initializer(1.0, 0.02))
        center = tf.compat.v1.get_variable("center", [nchannels], initializer=tf.constant_initializer(0.0))
        ave, dev = tf.nn.moments(x, axes=[1,2], keepdims=True)
        inv_dev = tf.math.rsqrt(dev + eps)
